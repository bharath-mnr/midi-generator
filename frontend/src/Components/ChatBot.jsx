...
const ChatBot = ({ isAuthenticated, user, onOpenAuth, onNavigate, onLogout }) => {
    ...
    try {
      ...
      const loadedMessages = response.data.map(chat => ([
        ...
        },
        {
          ...
          content: chat.botResponse,
          midiUrl: chat.midiUrl,
          generationId: chat.id, // ✅ ADDED: Store generation ID from chat history
          barCount: chat.generatedBars,
          source: chat.source || 'web',
          timestamp: chat.createdAt
        }
  ...
  /**
   * ✅ FIXED: Download MIDI using generation ID instead of direct file URL
   * @param {string|number} generationIdOrUrl - Can be ID or legacy URL
   * @param {string} filename - Optional filename for download
   */
  const downloadMidi = async (generationIdOrUrl, filename = null) => {
    try {
      console.log('🎵 Download requested:', generationIdOrUrl);

      let downloadUrl;
      let downloadFilename = filename || `composition_${Date.now()}.mid`;

      // ✅ Check if it's a legacy URL (contains '/generated/') or new ID
      if (typeof generationIdOrUrl === 'string' && generationIdOrUrl.includes('/generated/')) {
        // Legacy format: direct file URL
        console.log('⚠️ Using legacy download format');
        downloadUrl = generationIdOrUrl;
      } else if (typeof generationIdOrUrl === 'string' && generationIdOrUrl.includes('/api/midi/download/')) {
        // Already formatted URL
        downloadUrl = generationIdOrUrl;
      } else {
        // ✅ NEW FORMAT: Use ID to construct download URL
        const generationId = generationIdOrUrl;
        downloadUrl = `/api/midi/download/${generationId}`;
        console.log('✅ Using new ID-based download:', downloadUrl);
      }
      ...
      addErrorMessage('Failed to download MIDI file. Please try again.');
    }
  };

  /**
   * ✅ ALTERNATIVE: Direct download via Axios (more control)
   * Use this if you need to handle authentication or show progress
   */
  const downloadMidiWithAxios = async (generationId, filename = null) => {
    try {
      console.log('🎵 Downloading MIDI with authentication:', generationId);

      const response = await axiosInstance.get(`/midi/download/${generationId}`, {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = filename || `composition_${Date.now()}.mid`;
      document.body.appendChild(link);
      link.click();
      ...
      if (error.response?.status === 404) {
        addErrorMessage('MIDI file not found. It may have been deleted.');
      } else if (error.response?.status === 401) {
        addErrorMessage('Please log in to download MIDI files.');
        onOpenAuth('login');
      } else {
        addErrorMessage('Failed to download MIDI file. Please try again.');
      }
  ...
  // ✅ UPDATED: sendMessage function to handle new response format
  const sendMessage = async (messageType = 'generate') => {
    // Check server status first
    if (serverStatus !== 'healthy') {
      addErrorMessage('⚠️ Server is starting up. Please wait until the server is ready (1-3 minutes for first visit)');
    ...
    try {
      ...
      // ✅ FIXED: Extract generation ID from response
      const generationId = data.id; // This comes from MidiGenerationProxyService
      const midiUrl = data.midiUrl; // This is the download URL

      const botMessage = {
        id: data.id || Date.now(),
        type: 'bot',
        content: data.message,
        midiUrl: midiUrl, // Full download URL
        generationId: generationId, // ✅ NEW: Store ID separately
        barCount: data.barCount,
        wasUploadBased: currentUpload !== null,
        source: data.source || 'web',
        timestamp: data.timestamp
...
/**
 * ✅ UPDATED: MessageBubble component with fixed download
 */
const MessageBubble = ({ message, onDownload }) => {
  const getSourceBadge = (source) => {
    if (source === 'vst' || source === 'VST') {
  ...
  // ✅ FIXED: Bot message with correct download handling
  return (
    <div className="flex justify-start">
      <div className="bg-white rounded-2xl shadow-sm max-w-[90%] sm:max-w-[80%] overflow-hidden border">
        <div className="px-3 py-2">
          ...
          {message.midiUrl && (
            <div className={`rounded-lg p-3 mb-3 ${
              ...
              <div className="flex items-center justify-between gap-3 flex-col sm:flex-row">
                ...
                {/* ✅ FIXED: Download button using generation ID */}
                <button
                  onClick={() => {
                    // Use ID if available, fallback to URL
                    if (message.generationId) {
                      onDownload(message.generationId);
                    } else {
                      onDownload(message.midiUrl);
                    }
                  }}
                  className={`w-full sm:w-auto px-3 py-2 rounded-lg text-sm whitespace-nowrap flex items-center justify-center space-x-2 transition-colors ${
                    (message.source === 'vst' || message.source === 'VST')

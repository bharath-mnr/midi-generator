...
const useServerStatus = () => {
  ...
  useEffect(() => {
    const unsubscribe = serverStatusService.subscribe((newStatus) => {
      setStatus(newStatus);
      // Only show banner for problematic states, not for 'checking' which is quick
      setShowBanner(newStatus === 'waking' || newStatus === 'error');
    });
...
// Improved Server Status Banner - Much Less Intrusive
const ServerStatusBanner = ({ status, onRetry, onDismiss, showBanner }) => {
  ...
  const configs = {
    waking: {
      ...
      message: 'First-time setup may take 30-60 seconds. Your experience will be ready shortly...',
      action: null
    },
    error: {
      icon: AlertCircle,
      title: 'Connection issue',
      message: 'Having trouble connecting. We\'ll automatically retry.',
      action: 'Retry Now'
    }
  };
...
const ChatBot = ({ isAuthenticated, user, onOpenAuth, onNavigate, onLogout }) => {
  // Improved server status with banner control
  const { 
    status: serverStatus, 
    showBanner: showServerBanner,
    retry: retryServer, 
    dismissBanner: dismissServerBanner 
  } = useServerStatus();
  ...
  return (
    <div className="flex h-screen bg-white overflow-hidden text-sm safe-area">
      ...
      {/* Server Status Banner - Much less intrusive */}
      <ServerStatusBanner
        status={serverStatus}
        onRetry={retryServer}
              ...
              {serverStatus !== 'healthy' && (
                <div className="flex items-center space-x-1 text-xs text-gray-500">
                  ...
                  <span className="hidden sm:inline">
                    {serverStatus === 'waking' ? 'Starting up...' : 'Connecting...'}
                  </span>
                </div>
              )}
            </div>

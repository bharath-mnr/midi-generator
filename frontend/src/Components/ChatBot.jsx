  ...
  const sendMessage = async (messageType = 'generate') => {
    ...
    try {
      ...
      if (error.response?.status === 400) {
        const errorMsg = error.response?.data?.message || error.response?.data?.details || '';
      ...
      if (error.response?.status === 504) {
        const details = error.response?.data?.details || 'Generation took too long';
        const suggestion = error.response?.data?.suggestion || 'Try reducing the number of bars';
        addErrorMessage(`⏱️ ${details}. ${suggestion}.`);
        return;
      }
      ...
      if (error.response?.status === 500) {
        ...
        // ✅ FIX: Only show "waking up" for actual startup issues
        if (details.includes('service issues') || details.includes('high demand')) {
          addErrorMessage(`⚠️ ${details} ${suggestion}`);
        } else if (details.includes('invalid output')) {
          addErrorMessage('❌ The AI generated invalid output. Please try a different prompt or reduce complexity.');
        } else {
          // Generic server error
          addErrorMessage('❌ Server error occurred. Please try again with a simpler prompt or fewer bars.');
        }
        return;
      }
      ...
      // Generic fallback
      const fallbackMsg = error.response?.data?.details || 
                         error.response?.data?.message || 
                         error.message || 
                         'Something went wrong';
      
      addErrorMessage(`❌ ${fallbackMsg}. Please try again. If the problem persists, contact support.`);

...
// frontend/src/Components/UserDashboard.jsx
// ✅ FIXED: Updated download URLs to use generation IDs
...
const UserDashboard = () => {
  ...
  /**
   * ✅ NEW: Download MIDI file using generation ID
   * @param {number} generationId - Database ID
   * @param {string} filename - Optional filename
   */
  const downloadMidi = async (generationId, filename = null) => {
    try {
      console.log('🎵 Downloading MIDI ID:', generationId);

      // ✅ Use ID-based download URL
      const downloadUrl = `/api/midi/download/${generationId}`;

      // Create download link
      const link = document.createElement('a');
      link.href = downloadUrl;
      link.download = filename || `composition_${generationId}.mid`;
  ...
  /**
   * ✅ ALTERNATIVE: Download with Axios (shows progress, handles auth)
   */
  const downloadMidiWithAxios = async (generationId, filename = null) => {
    try {
      console.log('🎵 Downloading with auth:', generationId);
      ...
      if (error.response?.status === 404) {
        alert('MIDI file not found. It may have been deleted.');
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      <button
                        onClick={() => downloadMidi(generation.id, generation.fileName)}
                        className="p-1.5 sm:p-2 hover:bg-gray-100 rounded-lg transition-colors"
                        title="Download MIDI"
                      >
                        <Download className="w-4 h-4 text-gray-600" />

    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-3 sm:py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900">MIDI Generator</h1>
            <div className="flex items-center space-x-2 sm:space-x-4">
              {isLoggedIn ? (
                <>
                  <button
                    onClick={() => window.location.href = '/dashboard'}
                    className="px-2 sm:px-3 md:px-4 py-1.5 sm:py-2 text-sm sm:text-base text-gray-700 hover:text-gray-900 font-medium"
                  >
                    Dashboard
                  </button>
                  <button
                    onClick={() => window.location.href = '/Generator'}
                    className="px-2 sm:px-3 md:px-4 py-1.5 sm:py-2 text-sm sm:text-base bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition-colors"
                  >
                    Create Music
                  </button>
                </>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-8 sm:py-12 md:py-16 lg:py-20">
        <div className="text-center mb-8 sm:mb-12 md:mb-16">
          ...
          <p className="text-base sm:text-lg md:text-xl text-gray-600 max-w-3xl mx-auto mb-4 sm:mb-6 md:mb-8 px-4">
            Choose the plan that fits your music creation needs. More generations = more creativity. That's it.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 md:gap-6 mb-8 sm:mb-12 md:mb-16">
          {plans.map((plan) => {
            ...
            return (
              <div
                key={plan.tier}
                className={`relative rounded-xl sm:rounded-2xl overflow-hidden transition-all ${
                  isRecommended
                    ? 'ring-2 ring-gray-900 md:transform md:scale-105'
                    : ''
                } ${isCurrentPlan ? 'bg-gray-50' : 'bg-white'} shadow-lg hover:shadow-xl`}
              >
                {/* Recommended Badge */}
                {isRecommended && (
                  <div className="absolute top-0 right-0 bg-gradient-to-r from-gray-900 to-gray-700 text-white px-2 sm:px-3 md:px-4 py-0.5 sm:py-1 text-[10px] sm:text-xs font-semibold rounded-bl-lg">
                    RECOMMENDED
                  </div>
                )}
                ...
                {/* Current Plan Badge */}
                {isCurrentPlan && (
                  <div className="absolute top-0 left-0 bg-green-500 text-white px-2 sm:px-3 md:px-4 py-0.5 sm:py-1 text-[10px] sm:text-xs font-semibold rounded-br-lg">
                    YOUR PLAN
                  </div>
                )}
                ...
                <div className="p-4 sm:p-6 md:p-8">
                  ...
                  <div className="mb-4 sm:mb-5 md:mb-6">
                    ...
                    {plan.tier === 'UNLIMITED' && (
                      <Sparkles className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 text-orange-600" />
                    )}
                  </div>
                  ...
                  <div className="mb-4 sm:mb-5 md:mb-6">
                    {plan.monthlyPrice === 0 ? (
                      ...
                      <div>
                        <span className="text-3xl sm:text-4xl font-bold text-gray-900">${plan.monthlyPrice}</span>
                  ...
                  {/* Main Feature - Generations */}
                  <div className="mb-4 sm:mb-5 md:mb-6 pb-4 sm:pb-5 md:pb-6 border-b-2 border-gray-100">
                  ...
                  <button
                    onClick={() => handleSelectPlan(plan)}
                    className={`w-full py-2.5 sm:py-3 rounded-lg font-semibold transition-all mb-4 sm:mb-5 md:mb-6 flex items-center justify-center gap-2 text-sm sm:text-base ${
                      isCurrentPlan
                        ? 'bg-gray-200 text-gray-700 cursor-not-allowed'
                        : isRecommended
                        ? 'bg-gray-900 text-white hover:bg-gray-800'
        ...
        {/* CTA Section */}
        {!isLoggedIn && (
          <div className="mt-8 sm:mt-12 md:mt-16 bg-gradient-to-r from-gray-900 to-gray-800 rounded-xl sm:rounded-2xl p-6 sm:p-8 md:p-12 text-center">
            ...
            <button
              onClick={() => window.location.href = '/signup'}
              className="px-6 sm:px-8 py-3 sm:py-4 bg-white text-gray-900 rounded-lg font-semibold text-base sm:text-lg hover:bg-gray-100 transition-colors"
            >
              Get Started Free
            </button>
          </div>
...
const FAQItem = ({ question, answer }) => {
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
      ...
      >
        <span className="font-semibold text-sm sm:text-base text-gray-900">{question}</span>
        <span className={`transform transition-transform ${isOpen ? 'rotate-180' : ''}`}>
          ▼
        </span>
      </button>

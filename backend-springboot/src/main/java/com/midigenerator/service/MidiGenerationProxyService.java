...
public class MidiGenerationProxyService {
    ...
    /**
     * ✅ MODIFIED: Main entry point - now stores text MIDI in database
     * Physical files still created but text is primary source of truth
     */
    @Transactional
    public Map<String, Object> generateMidi(GenerationRequest request) {
        User user = userService.getCurrentUser();
        ...
        try {
            ...
            lockedUser.decrementGenerationCount();
            userRepository.saveAndFlush(lockedUser);
            log.info("↩️ Rolled back generation count for user: {}", lockedUser.getEmail());
            throw new RuntimeException("Failed to generate MIDI: " + e.getMessage());
        }
    }
    ...
    /**
     * ✅ MODIFIED: Performs generation and stores text MIDI in database
     */
    private Map<String, Object> performGeneration(GenerationRequest request, User user) {
        String originalContentToUse = request.getOriginalContent();
        ...
        nodeRequest.put("editMode", request.getEditMode());
        nodeRequest.put("originalContent", originalContentToUse);
        // ✅ NEW: Still create physical file for now (hybrid approach)
        nodeRequest.put("returnTextOnly", false);
        ...
        // ✅ MODIFIED: Save generation with text MIDI to database
        MidiGeneration saved = saveMidiGenerationToDatabase(request, user, nodeResponse);
    ...
    /**
     * ✅ NEW: Saves MIDI generation with text notation to database
     * Also creates download URL pointing to on-demand conversion endpoint
     */
    private MidiGeneration saveMidiGenerationToDatabase(GenerationRequest request, User user, Map<String, Object> nodeResponse) {
        MidiGeneration generation = new MidiGeneration();
        generation.setUser(user);
        generation.setFileName("generated_" + System.currentTimeMillis() + ".mid");
        generation.setOriginalPrompt(request.getMessage());
        ...
        // ✅ Store text MIDI notation in database (PRIMARY SOURCE)
        String textMidi = (String) nodeResponse.get("message");
        generation.setTextMidi(textMidi);
        ...
        // ✅ NEW: Create download URL pointing to on-demand conversion endpoint
        String downloadUrl = baseUrl + "/api/midi/download/" + saved.getId();
        saved.setMidiUrl(downloadUrl);
        
        // Save again with updated URL
        saved = midiGenerationRepository.save(saved);

        log.info("💾 Saved text MIDI to database with ID: {} from {} ({} bars)", 
                 saved.getId(), saved.getSource(), barCount);
    ...
    /**
     * ✅ NEW: Public method for uploading MIDI (used by controller)
     */
    public Map<String, Object> uploadMidi(Map<String, Object> uploadRequest) {
        try {
            HttpHeaders headers = new HttpHeaders();
